...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (c <= 1) {
      SibTreeNode oldFirst = firstChild;
      firstChild = newChild;
      newChild.nextSibling = oldFirst;
    } else if (numChildren < c) {
      ((SibTreeNode) child(numChildren)).nextSibling = newChild;
    } else {
      SibTreeNode leftSib = (SibTreeNode) child(c - 1);
      SibTreeNode rightSib = (SibTreeNode) child(c);
      leftSib.nextSibling = newChild;
      newChild.nextSibling = rightSib;
    }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (children() == 0) {
      if (parent == null) {
        ...
        SibTreeNode leftSib = parent.firstChild;
        SibTreeNode rightSib = this.nextSibling;
        while (leftSib.nextSibling != this) {
          leftSib = leftSib.nextSibling;
        }
